...
shoppingCart.prototype.addItem = function (idProduct, name, price, quantity) {
...
shoppingCart.prototype.getTotalPrice = function (idProduct) {
    var total = 0;
...
// get the total price for all items currently in the cart
shoppingCart.prototype.getTotalCount = function (idProduct) {
    var count = 0;
    for (var i = 0; i < this.items.length; i++) {
        var item = this.items[i];
        if (idProduct == null || item.id == idProduct)
...
shoppingCart.prototype.checkoutPagSeguro = function (parms, clearCart) {
